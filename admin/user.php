...
if (isset($_POST['add_user'])) {
    ...
    $password = $_POST['password']; // Masih dalam bentuk plain text
    $role = trim($_POST['role']);

    // Cek apakah username sudah ada
    $cek_sql = "SELECT COUNT(*) FROM tb_user WHERE username = ?";
...
if (isset($_POST['update_user'])) {
    ...
    $cek_sql = "SELECT COUNT(*) FROM tb_user WHERE username = ? AND id_user != ?";
    $cek_stmt = $config->prepare($cek_sql);
    $cek_stmt->bind_param("si", $username, $id_user);
    $cek_stmt->execute();
    $cek_stmt->bind_result($count);
    $cek_stmt->fetch();
    $cek_stmt->close();
    ...
    $sql = "UPDATE tb_user SET username = ?, password = ?, role = ? WHERE id_user = ?";
    $stmt = $config->prepare($sql);
...
$query_count = "SELECT COUNT(*) AS total FROM tb_user WHERE id_user LIKE '%$search%'";
...
<body id="page-top">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    $sql = "SELECT * FROM tb_user WHERE 1";

                                    // Jika ada pencarian, tambahkan kondisi WHERE
                                    if (!empty($search)) {
                                        $sql .= " AND username LIKE '%" . mysqli_real_escape_string($config, $search) . "%'";
                                    }
    ...
    <a href='?delete={$row['id_user']}' class='btn btn-transparent btn-sm' onclick='return confirm(\"Hapus data ini?\")'>
        <i class='fas fa-trash-alt'></i>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="username" class="form-control" id="username_edit<?= $row['id_user']; ?>"
                                                    value="<?= $row['username']; ?>" required
                                                    oninput="validateUsername(this, 'errorUsernameEdit<?= $row['id_user']; ?>')">
                                                <small id="errorUsernameEdit<?= $row['id_user']; ?>" class="text-danger" style="display:none;">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="username" class="form-control" id="username" required
                                                ...
                                                oninput="validateUsername(this, 'errorUsername')">
                                            <small id="errorUsername" class="text-danger" style="display:none;">
                                                Username hanya boleh berisi huruf!
                                            </small>
                                        </div>
                    ...
                    <script>
                        // Validasi Username hanya huruf tanpa spasi
                        ...
                        function validateForm() {
                            ...
                            if (!/^[a-zA-Z]+$/.test(input.value)) {
                                alert("Gagal menambahkan data! Username hanya boleh berisi huruf.");
