...
if (isset($_POST['add_kategori'])) {
    $namakategori = trim($_POST['nama_kategori']); // Hilangkan spasi di awal & akhir

    // Validasi hanya huruf & spasi
    if (!preg_match("/^[a-zA-Z\s]+$/", $namakategori)) {
        echo "<script>alert('Kategori hanya boleh berisi huruf!'); window.history.back();</script>";
        exit();
    }

    // Cek apakah kategori sudah ada
    $cek_sql = "SELECT COUNT(*) FROM tb_kategori WHERE nama_kategori = ?";
    $cek_stmt = $config->prepare($cek_sql);
    $cek_stmt->bind_param("s", $namakategori);
    $cek_stmt->execute();
...
if (isset($_POST['update_kategori'])) {
    ...
    $nama_kategori = trim($_POST['nama_kategori']); // Hilangkan spasi di awal & akhir

    // Validasi hanya huruf & spasi
    if (!preg_match("/^[a-zA-Z\s]+$/", $nama_kategori)) {
        echo "<script>alert('Kategori hanya boleh berisi huruf dan spasi!'); window.history.back();</script>";
        exit();
    }

    // Cek apakah kategori yang baru sudah ada di database
    $cek_sql = "SELECT COUNT(*) FROM tb_kategori WHERE nama_kategori = ? AND id_kategori != ?";
    $cek_stmt = $config->prepare($cek_sql);
    $cek_stmt->bind_param("si", $nama_kategori, $id_kategori);
    $cek_stmt->execute();
    $cek_stmt->bind_result($count);
    $cek_stmt->fetch();
    $cek_stmt->close();
    ...
    if ($count > 0) {
        echo "<script>alert('Nama kategori sudah digunakan!'); window.history.back();</script>";
        exit();
    }
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Data berhasil diperbarui!'); window.location.href='kategori.php';</script>";
    } else {
        echo "<script>alert('Gagal memperbarui data: " . $config->error . "');</script>";
    }
...
$query = "SELECT * FROM tb_kategori WHERE nama_kategori LIKE '%$search%'";
if ($limit !== 'all') {
...
$query_count = "SELECT COUNT(*) AS total FROM tb_kategori WHERE nama_kategori LIKE '%$search%'";
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <!-- Konten Utama -->
                    <h1 class="fontuser">Data Kategori</h1>
                    <br>
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    $sql = "SELECT * FROM tb_kategori WHERE 1";

                                    // Jika ada pencarian, tambahkan kondisi WHERE
                                    if (!empty($search)) {
                                        $sql .= " AND nama_kategori LIKE '%" . mysqli_real_escape_string($config, $search) . "%'";
                                    }
            ...
            <td>
                <button class='btn btn-transparent btn-sm' data-toggle='modal' data-target='#editDataModal{$row['id_kategori']}'>
                    <i class='fas fa-edit'></i>
                </button>
                <a href='?delete={$row['id_kategori']}' class='btn btn-transparent btn-sm' onclick='return confirm(\"Hapus data ini?\")'>
                    ...
                    <?php foreach ($result as $row) : ?>
                        <div class="modal fade" id="editDataModal<?= $row['id_kategori']; ?>" tabindex="-1" aria-labelledby="editDataModalLabel" aria-hidden="true">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="nama_kategori" class="form-control" id="nama_kategori_edit<?= $row['id_kategori']; ?>" value="<?= $row['nama_kategori']; ?>" required oninput="validateNamaKategori(this, 'errorNamaEdit<?= $row['id_kategori']; ?>')">
                                                <small id="errorNamaEdit<?= $row['id_kategori']; ?>" class="text-danger" style="display:none;">Kategori hanya boleh berisi huruf!</small>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="submit" name="update_kategori" class="btn btn-primary">Simpan</button>
                            ...
                            <form method="POST" onsubmit="return validateForm()">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            <label>Kategori</label>
                                            <input type="text" name="nama_kategori" class="form-control" id="nama_kategori" required placeholder="Masukkan Kategori" oninput="validateNamaKategori(this, 'errorNama')">
                                            <small id="errorNama" class="text-danger" style="display:none;">Kategori hanya boleh berisi huruf!</small>
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                    ...
                    <script>
                        function validateNamaKategori(input, errorId) {
                            let regex = /^[a-zA-Z\s]+$/;
                        ...
                        function validateForm() {
                            let input = document.getElementById("nama_kategori");
                            let errorElement = document.getElementById("errorNama");

                            // Jika input salah, hapus isinya dan tolak submit
                            if (!/^[a-zA-Z\s]+$/.test(input.value)) {
                                alert("Gagal menambahkan data! Kategori hanya boleh berisi huruf.");
