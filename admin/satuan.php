...
if (isset($_POST['add_satuan'])) {
    ...
    if (!preg_match("/^[a-zA-Z\s]+$/", $namasatuan)) {
        echo "<script>alert('Satuan hanya boleh berisi huruf dan spasi!'); window.history.back();</script>";
        exit();
    }

    // Cek apakah satuan sudah ada
    $cek_sql = "SELECT COUNT(*) FROM tb_satuan WHERE nama_satuan = ?";
    $cek_stmt = $config->prepare($cek_sql);
    $cek_stmt->bind_param("s", $namasatuan);
    $cek_stmt->execute();
    ...
    if ($count > 0) {
        echo "<script>alert('Satuan sudah ada!'); window.history.back();</script>";
        exit();
    }

    // Insert ke database menggunakan prepared statement
    $sql = "INSERT INTO tb_satuan (nama_satuan) VALUES (?)";
    $stmt = $config->prepare($sql);
    $stmt->bind_param("s", $namasatuan);

    if ($stmt->execute()) {
        echo "<script>alert('Data berhasil ditambahkan!'); window.location.href='satuan.php';</script>";
    } else {
...
if (isset($_POST['update_satuan'])) {
    ...
    $namasatuan = trim($_POST['nama_satuan']); // Hilangkan spasi di awal & akhir
    ...
    // Cek apakah satuan yang baru sudah ada di database
    $cek_sql = "SELECT COUNT(*) FROM tb_satuan WHERE nama_satuan = ? AND id_satuan != ?";
    $cek_stmt = $config->prepare($cek_sql);
    $cek_stmt->bind_param("si", $namasatuan, $id_satuan);
    $cek_stmt->execute();
    $cek_stmt->bind_result($count);
    $cek_stmt->fetch();
    $cek_stmt->close();
    ...
    if ($count > 0) {
        echo "<script>alert('Nama satuan sudah digunakan!'); window.history.back();</script>";
        exit();
    }

    // Update data di database menggunakan prepared statement
    $sql = "UPDATE tb_satuan SET nama_satuan = ? WHERE id_satuan = ?";
    $stmt = $config->prepare($sql);
    $stmt->bind_param("si", $namasatuan, $id_satuan);

    if ($stmt->execute()) {
        echo "<script>alert('Data berhasil diperbarui!'); window.location.href='satuan.php';</script>";
    } else {
...
if (isset($_GET['delete'])) {
    ...
    if ($config->query($sql)) {
        echo "<script>alert('Data berhasil di hapus!'); window.location.href='satuan.php';</script>";
    } else {
...
if (isset($_GET['id_satuan'])) {
    ...
    $sql = "SELECT * FROM tb_satuan WHERE id_satuan = '$id_satuan'";
...
$query = "SELECT * FROM tb_satuan WHERE nama_satuan LIKE '%$search%'";
if ($limit !== 'all') {
...
$query_count = "SELECT COUNT(*) AS total FROM tb_satuan WHERE nama_satuan LIKE '%$search%'";
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <!-- Konten Utama -->
                    <h1 class="fontuser">Data Satuan</h1>
                    <br>
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-header py-3 d-flex justify-content-between align-items-center">
                            ...
                            <form method="GET" class="form-inline">
                                <input type="text" name="search" class="form-control" placeholder="Cari Satuan" value="<?= htmlspecialchars($search); ?>">
                            </form>
                        </div>
                        <div class="card-body">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    $sql = "SELECT * FROM tb_satuan WHERE 1";

                                    // Jika ada pencarian, tambahkan kondisi WHERE
                                    if (!empty($search)) {
                                        $sql .= " AND nama_satuan LIKE '%" . mysqli_real_escape_string($config, $search) . "%'";
                                    }

                                    // Tambahkan ORDER BY
                                    $sql .= " ORDER BY id_satuan DESC";
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            echo "<tr>
                                <td>$no</td>
                                <td>{$row['nama_satuan']}</td> <!-- Menampilkan nama kategori -->
                                <td>
                                    <button class='btn btn-transparent btn-sm' data-toggle='modal' data-target='#editDataModal{$row['id_satuan']}'>
                                        <i class='fas fa-edit'></i>
                                    </button>
                                    <a href='?delete={$row['id_satuan']}' class='btn btn-transparent btn-sm' onclick='return confirm(\"Hapus data ini?\")'>
                                        ...
                                        echo "<tr><td colspan='4' class='text-center'>Tidak ada data satuan</td></tr>";
                                    }
                                    ?>
                                </tbody>
                            </table>
                    ...
                    <?php foreach ($result as $row) : ?>
                        <div class="modal fade" id="editDataModal<?= $row['id_satuan']; ?>" tabindex="-1" aria-labelledby="editDataModalLabel" aria-hidden="true">
                                ...
                                <form method="POST">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                <label>Satuan</label>
                                                <input type="text" name="nama_satuan" class="form-control" id="nama_satuan_edit<?= $row['id_satuan']; ?>" value="<?= $row['nama_satuan']; ?>" required oninput="validateNamasatuan(this, 'errorNamaEdit<?= $row['id_satuan']; ?>')">
                                                <small id="errorNamaEdit<?= $row['id_satuan']; ?>" class="text-danger" style="display:none;">satuan hanya boleh berisi huruf!</small>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="submit" name="update_satuan" class="btn btn-primary">Simpan</button>
                            ...
                            <form method="POST" onsubmit="return validateForm()">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            <label>Satuan</label>
                                            <input type="text" name="nama_satuan" class="form-control" id="nama_satuan" required placeholder="Masukkan satuan" oninput="validateNamasatuan(this, 'errorNama')">
                                            <small id="errorNama" class="text-danger" style="display:none;">Satuan hanya boleh berisi huruf!</small>
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="submit" name="add_satuan" class="btn btn-primary">Simpan</button>
                    ...
                    <script>
                        function validateNamasatuan(input, errorId) {
                            let regex = /^[a-zA-Z\s]+$/;
                        ...
                        function validateForm() {
                            let input = document.getElementById("nama_satuan");
                            let errorElement = document.getElementById("errorNama");

                            // Jika input salah, hapus isinya dan tolak submit
                            if (!/^[a-zA-Z\s]+$/.test(input.value)) {
                                alert("Gagal menambahkan data! Satuan hanya boleh berisi huruf.");
