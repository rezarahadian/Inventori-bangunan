...
if (isset($_POST['add_supplier'])) {
    $namasupplier = trim($_POST['nama_supplier']); // Hilangkan spasi di awal & akhir
    $notelepon = trim($_POST['no_telepon']);
    $alamat = trim($_POST['alamat']);

    // Validasi nama supplier hanya boleh berisi huruf dan spasi
    if (!preg_match("/^[a-zA-Z\s]+$/", $namasupplier)) {
        echo "<script>alert('Nama supplier hanya boleh berisi huruf dan spasi!'); window.history.back();</script>";
        exit();
    }
    ...
    $cek_sql = "SELECT COUNT(*) FROM tb_supplier WHERE nama_supplier = ?";
    $cek_stmt = $config->prepare($cek_sql);
    $cek_stmt->bind_param("s", $namasupplier);
    $cek_stmt->execute();
    ...
    if ($count > 0) {
        echo "<script>alert('Supplier sudah ada!'); window.history.back();</script>";
        exit();
    }

    // Insert ke database menggunakan prepared statement
    $sql = "INSERT INTO tb_supplier (nama_supplier, no_telepon, alamat) VALUES (?, ?, ?)";
    $stmt = $config->prepare($sql);
    $stmt->bind_param("sss", $namasupplier, $notelepon, $alamat);
...
if (isset($_POST['update_supplier'])) {
    ...
    // Cek apakah nama supplier yang baru sudah ada di database (selain dirinya sendiri)
    $cek_sql = "SELECT COUNT(*) FROM tb_supplier WHERE nama_supplier = ? AND id_supplier != ?";
    $cek_stmt = $config->prepare($cek_sql);
    $cek_stmt->bind_param("si", $namasupplier, $id_supplier);
    $cek_stmt->execute();
    $cek_stmt->bind_result($count);
    $cek_stmt->fetch();
    $cek_stmt->close();
    ...
    if ($count > 0) {
        echo "<script>alert('Nama supplier sudah digunakan!'); window.history.back();</script>";
        exit();
    }

    // Update data di database menggunakan prepared statement
    $sql = "UPDATE tb_supplier SET nama_supplier = ?, no_telepon = ?, alamat = ? WHERE id_supplier = ?";
    $stmt = $config->prepare($sql);
    $stmt->bind_param("sssi", $namasupplier, $notelepon, $alamat, $id_supplier);
...
if (isset($_GET['delete'])) {
    ...
    if ($config->query($sql)) {
        echo "<script>alert('Data berhasil di hapus'); window.location.href='supplier.php';</script>";
    } else {
...
$query = "SELECT * FROM tb_supplier WHERE nama_supplier LIKE '%$search%'";
if ($limit !== 'all') {
...
$query_count = "SELECT COUNT(*) AS total FROM tb_supplier WHERE nama_supplier LIKE '%$search%'";
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <!-- Konten Utama -->
                    <h1 class="fontuser">Data Supplier</h1>
                    <br>
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    if (!empty($search)) {
                                        $sql .= " AND nama_supplier LIKE '%" . mysqli_real_escape_string($config, $search) . "%'";
                                    }

                                    // Tambahkan ORDER BY
                                    $sql .= " ORDER BY id_supplier DESC";
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            echo "<tr>
                                <td>$no</td>
                                <td>{$row['nama_supplier']}</td> <!-- Menampilkan nama kategori -->
                                 <td>{$row['no_telepon']}</td>
                                  <td>{$row['alamat']}</td>
                                <td>
                                    <button class='btn btn-transparent btn-sm' data-toggle='modal' data-target='#editDataModal{$row['id_supplier']}'>
                                        <i class='fas fa-edit'></i>
                                    </button>
                                    <a href='?delete={$row['id_supplier']}' class='btn btn-transparent btn-sm' onclick='return confirm(\"Hapus data ini?\")'>
                                        ...
                                        echo "<tr><td colspan='4' class='text-center'>Tidak ada data Supplier</td></tr>";
                                    }
                                    ?>
                    ...
                    <?php foreach ($result as $row) : ?>
                        <div class="modal fade" id="editDataModal<?= $row['id_supplier']; ?>" tabindex="-1" aria-labelledby="editDataModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <form method="POST" onsubmit="return validateForm('nama_supplier_edit<?= $row['id_supplier']; ?>', 'no_telepon_edit<?= $row['id_supplier']; ?>', 'alamat_edit<?= $row['id_supplier']; ?>', 'namaSupplierErrorEdit<?= $row['id_supplier']; ?>', 'noTeleponErrorEdit<?= $row['id_supplier']; ?>', 'alamatErrorEdit<?= $row['id_supplier']; ?>')">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="nama_supplier" class="form-control"
                                                    id="nama_supplier_edit<?= $row['id_supplier']; ?>"
                                                    value="<?= $row['nama_supplier']; ?>" required
                                                    oninput="validateNama(this, 'namaSupplierErrorEdit<?= $row['id_supplier']; ?>')">
                                                <small id="namaSupplierErrorEdit<?= $row['id_supplier']; ?>" class="text-danger" style="display:none;">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="no_telepon" class="form-control"
                                                    id="no_telepon_edit<?= $row['id_supplier']; ?>"
                                                    value="<?= $row['no_telepon']; ?>" required
                                                    oninput="validateTelepon(this, 'noTeleponErrorEdit<?= $row['id_supplier']; ?>')">
                                                <small id="noTeleponErrorEdit<?= $row['id_supplier']; ?>" class="text-danger" style="display:none;">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="alamat" class="form-control"
                                                    id="alamat_edit<?= $row['id_supplier']; ?>"
                                                    value="<?= $row['alamat']; ?>" required
                                                    oninput="validateAlamat(this, 'alamatErrorEdit<?= $row['id_supplier']; ?>')">
                                                <small id="alamatErrorEdit<?= $row['id_supplier']; ?>" class="text-danger" style="display:none;">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button class="btn btn-secondary" type="button" data-dismiss="modal" onclick="resetForm('nama_supplier_edit<?= $row['id_supplier']; ?>', 'no_telepon_edit<?= $row['id_supplier']; ?>', 'alamat_edit<?= $row['id_supplier']; ?>', 'namaSupplierErrorEdit<?= $row['id_supplier']; ?>', 'noTeleponErrorEdit<?= $row['id_supplier']; ?>', 'alamatErrorEdit<?= $row['id_supplier']; ?>')">Batal</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                    ...
                    <div class="modal fade" id="tambahDataModal" tabindex="-1" aria-labelledby="tambahDataModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <form method="POST" onsubmit="return validateForm('nama_supplier', 'no_telepon', 'alamat', 'namaSupplierError', 'noTeleponError', 'alamatError')">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="nama_supplier" class="form-control" required
                                                placeholder="Masukkan Nama Supplier" id="nama_supplier"
                                                oninput="validateNama(this, 'namaSupplierError')">
                                            <small id="namaSupplierError" class="text-danger" style="display:none;">
                                                Nama hanya boleh berisi huruf dan spasi!
                                            </small>
                                        </div>
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <button class="btn btn-secondary" type="button" data-dismiss="modal" onclick="resetForm('nama_supplier', 'no_telepon', 'alamat', 'namaSupplierError', 'noTeleponError', 'alamatError')">Batal</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                    ...
                    <script>
                        ...
                        function validateTelepon(input, errorId) {
                            let regex = /^[0-9]+$/;
                            let errorElement = document.getElementById(errorId);
                        ...
                        function validateForm(namaId, teleponId, alamatId, namaErrorId, teleponErrorId, alamatErrorId) {
                            ...
                            let alamatValid = /^[a-zA-Z0-9\s.,]+$/.test(alamat.value);
                            ...
                            if (!alamatValid) {
                                ...
                                errorMessages.push("Alamat tidak valid! Tidak boleh ada karakter khusus.");
